...
function completingActualCreditDebtTable() {
	...
	*/
	...
	for (
		...
		let debtTotalAccruedInterestDue = 0;
		...
		for (let i = 0; i < debtDate.length; i++) {
			...
			// ?Total Interest Due =========================================
			let index_lastObligation;
			// Finding the index of last "obligation"
			if (
				debtTypeOperation[i].innerHTML == 'obligation' &&
				new Date(dateStringForDateObject(debtDate[i].innerHTML)).getTime() <=
					new Date(
						dateStringForDateObject(
							scheduleDatePayment[scheduleDatePayment.length - 1].innerHTML
						)
					).getTime()
			) {
				index_lastObligation = i;
			}
			// Calculation of "Accrued Interest Due" and "net Interest Due"
			//  by using the index of last "obligation"
			if (index_lastObligation != undefined) {
				// debtTotalInterestDue = 0;
				debtTotalAccruedInterestDue = 0;
				for (let x = 0; x <= index_lastObligation; x++) {
					if (parseFloat(debtInterest[x].innerHTML) >= 0) {
						debtTotalAccruedInterestDue = (
							parseFloat(debtTotalAccruedInterestDue) +
							parseFloat(debtInterest[x].innerHTML)
						).toFixed(2);
					}
				}
			}
			debtTotalInterestDue = (
				parseFloat(debtTotalAccruedInterestDue) +
			...
			*/
			...
			if (debtTypeOperation[i].innerHTML == 'payment') {
				...
				// *Interest due payment (during the schedule period) ============
				...
				if (
					...
					if (
						// if tempDebtAmountPaid greater than debtTotalInterestDue and
						// debtTotalInterestDue is positive (not advance repayment event happen)
						parseFloat(tempDebtAmountPaid) >=
							parseFloat(debtTotalInterestDue) &&
						parseFloat(debtTotalInterestDue) >= 0
					) {
						debtInterest[i].innerHTML = (-parseFloat(
							debtTotalInterestDue
						)).toFixed(2);
						...
						tempDebtAmountPaid = (
							parseFloat(tempDebtAmountPaid) - parseFloat(debtTotalInterestDue)
						).toFixed(2);
						...
						// if tempDebtAmountPaid greater than debtTotalInterestDue and
						// debtTotalInterestDue is negative (advance repayment event happen).
						// In this situation debtTotalInterest = 0
						parseFloat(tempDebtAmountPaid) >=
							parseFloat(debtTotalInterestDue) &&
						parseFloat(debtTotalInterestDue) < 0
					) {
						...
						//if tempDebtAmountPaid less than debtTotalInterestDue
						parseFloat(tempDebtAmountPaid) < parseFloat(debtTotalInterestDue)
					) {
				...
				// *Interest payment (outside the schedule period) ===================
				if (
					// outside the schedule period
					new Date(dateStringForDateObject(debtDate[i].innerHTML)).getTime() >
					new Date(
						dateStringForDateObject(
							scheduleDatePayment[scheduleDatePayment.length - 1].innerHTML
...
window.addEventListener('load', () => {
	if (screen.width < 1024) {
		alert(
			'It is recommended to open the web page on a desktop device for more convenient viewing of tables, more precise adjustment of range sliders by using the arrow keys on the keyboard ("<--", "-->").'
		);
	}
});
//! Performance ====================================
